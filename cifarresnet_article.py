...
__all__ = ['get_cifar_resnet',
           ...
           'cifar_resnet20_v1_article','cifar_resnet56_v1_article','cifar_resnet110_v1_article']
...
class CIFARBasicBlockV1_article(HybridBlock):
    r"""BasicBlock V1 from `"Deep Residual Learning for Image Recognition"
    <http://arxiv.org/abs/1512.03385>`_ paper.
    This is used for ResNet V1 for 18, 34 layers.
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0,
                 norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        super(CIFARBasicBlockV1_article, self).__init__(**kwargs)
        self.body = nn.HybridSequential(prefix='')
        self.body.add(_conv3x3(channels, stride, in_channels))
        self.body.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
        self.body.add(nn.Activation('relu'))
...
class CIFARResNetV1_article(HybridBlock):
    r"""ResNet V1 model from
    `"Deep Residual Learning for Image Recognition"
    <http://arxiv.org/abs/1512.03385>`_ paper.
    ...
    def __init__(self, block, layers, channels, classes=10,
                 norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        super(CIFARResNetV1_article, self).__init__(**kwargs)
        assert len(layers) == len(channels) - 1
        with self.name_scope():
            self.features = nn.HybridSequential(prefix='')
            self.features.add(nn.Conv2D(channels[0], 3, 1, 1, use_bias=False))
    ...
    def _make_layer(self, block, layers, channels, stride, stage_index, in_channels=0,
        ...
        # if stride != 1 :
            ...
            # with downsample.name_scope():
                # downsample.add(nn.AvgPool2D(pool_size=stride, strides=stride,
                                            # ceil_mode=True, count_include_pad=False))
        ##
        layer = nn.HybridSequential(prefix='stage%d_'%stage_index)
        with layer.name_scope():
...
# Specification
resnet_net_versions = [CIFARResNetV1, CIFARResNetV2,CIFARResNetV1_article]
resnet_block_versions = [CIFARBasicBlockV1, CIFARBasicBlockV2,CIFARBasicBlockV1_article]
...
def get_cifar_resnet(version, num_layers, pretrained=False, ctx=cpu(),
    ...
    ----------
    version : int
        Version of ResNet. Options are 1, 2,3(article).
    num_layers : int
...
def cifar_resnet20_v1_article(**kwargs):
    r"""ResNet-20 V1 model for CIFAR10 from `"Deep Residual Learning for Image Recognition"
    <http://arxiv.org/abs/1512.03385>`_ paper.
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    return get_cifar_resnet(3, 20, **kwargs)


def cifar_resnet56_v1_article(**kwargs):
    r"""ResNet-56 V1 model for CIFAR10 from `"Deep Residual Learning for Image Recognition"
    <http://arxiv.org/abs/1512.03385>`_ paper.
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    return get_cifar_resnet(3, 56, **kwargs)


def cifar_resne110_v1_article(**kwargs):
    r"""ResNet-110 V1 model for CIFAR10 from `"Deep Residual Learning for Image Recognition"
    <http://arxiv.org/abs/1512.03385>`_ paper.
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    return get_cifar_resnet(3, 110, **kwargs)